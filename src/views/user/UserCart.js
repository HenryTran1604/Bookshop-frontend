...
const UserCartView = () => {
  ...
  const handlePurchase = () => {
    ...
    if (cartItems.length === 0) {
      setMessage([0, 'Vui lòng chọn sản phẩm!'])
      return
    }
    cartItems.forEach((cartItem) => {
      cartItem.purchaseStatus = "delivery"
      // cartItem.purchaseDate = ge
    })
    fetch('http://localhost:8081/api/purchase', {
      ...
      .then(response => {
        setMessage([1, 'Đơn hàng đã được đặt thành công!\nVui lòng kiểm tra trong mục đơn hàng'])
        fetchCart()
      })
      .catch(err => console.error(err))
  }
      ...
      <div className="container mb-3">
        <div className="row">
          <div className="col-md-9">
            ...
            <span className={message[0] === 0 ? "text-danger" : "text-success"}>{message[1]}</span>
